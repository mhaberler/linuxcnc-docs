...
*homed*:: '(returns tupple of integer)' +
(1, 1, 0, 0, 0, 1, 0, 0, 0)
flag. 1 if homed, +
so the example mean X,Y and C homed all others not
...
will return 1.0 for metric and
0.0393700787402 (1/25.4) for imperial 
...
`auto(int[, int])`:: 
	...
	Syntax: +
	auto(command, linenumber)
	command = AUTO_RUN ; AUTO_PAUSE ; AUTO_STEP ; AUTO_RESUME
	linenumber = only needed for AUTO_RUN if not omitted run from line 0 
...
`set_max_limit(int, float)`::
        ...
        Syntax: +
        set_max_limit(axis, value) +
        axis ; where 0 = x, 1 = y , 2 = z ... +
        value ; The value to set in machine units
...
`spindle(int, float)`::
	...
	Syntax: +
	spindle(direction, rpm = 0) +
	direction =  SPINDLE_FORWARD, SPINDLE_REVERSE, SPINDLE_OFF, +
	             SPINDLE_INCREASE, SPINDLE_DECREASE, or SPINDLE_CONSTANT. +
	rpm = the velocity to set, for SPINDLE_FORWARD, SPINDLE_REVERSE +
	      all other cases it should not be given

`spindleoverride(float)`::
	set spindle override factor 1.0 = 100 % ; 0.46 = 46 % 

`state(int)`::
	set the machine state. +
	Syntax: +
	state(state) +
	state = STATE_ESTOP, STATE_ESTOP_RESET, STATE_ON, or STATE_OFF
...
`traj_mode(int)`::
	...
	Syntax: +
	traj_mode(mode) +
	mode = MODE_FREE, MODE_COORD, MODE_TELEOP.
